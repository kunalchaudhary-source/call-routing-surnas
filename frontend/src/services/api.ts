...
export const api = {
  async listGreetings(): Promise<GreetingRecord[]> {
    const res = await fetch(`${API_BASE}/admin/greetings`);
    return handle(res);
  },
  async getGreeting(language: string): Promise<GreetingRecord> {
    const res = await fetch(`${API_BASE}/admin/greetings/${language}`);
    return handle(res);
  },
  ...
  async addSpecialization(agentId: number, payload: { category: string; proficiency_level: number }) {
    const res = await fetch(`${API_BASE}/admin/agents/${agentId}/specializations`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload)
    });
  ...
  async removeSpecialization(agentId: number, category: string) {
    const res = await fetch(`${API_BASE}/admin/agents/${agentId}/specializations/${category}`, {
      method: "DELETE"
    });
    return handle(res);
  },
  async listIVRPrompts(): Promise<IVRPromptRecord[]> {
    const res = await fetch(`${API_BASE}/admin/ivr-prompts`);
    return handle(res);
  },
  async upsertIVRPrompt(key: string, payload: { message: string }) {
    const res = await fetch(`${API_BASE}/admin/ivr-prompts/${key}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload)
    });

...
def _get_prompt(key: str) -> str:
    ...
    This keeps IVR text editable in the admin UI while allowing an optional
    Gemini rewrite for phone-friendly phrasing when `GEMINI_API_KEY` is set.
    """
...
async def voice(request: Request) -> Response:
    ...
    return Response(
        content=str(_build_menu_prompt(language_code, voice_name)),
...
async def voice_intent(request: Request) -> Response:
    ...
    intent = _resolve_intent(digits, speech_result)
    if not intent:
        log_event(call_sid, "INVALID_SELECTION", {"digits": digits, "speech": speech_result})
        response = _build_menu_prompt(language_code, voice_name)
        response.say(
            _get_prompt("invalid"),
            voice=voice_name,
            language=language_code,
    ...
    log_event(call_sid, "INTENT_SELECTED", {"intent": intent, "digits": digits, "speech": speech_result})

    # Branch depending on intent
    response = VoiceResponse()
    if intent in ("general_inquiry", "store"):
        ...
        gather.say(_get_prompt("assist_type_prompt"), voice=voice_name, language=language_code)
    ...
    if intent == "price_request":
        ...
        gather.say(_get_prompt("price_product_prompt"), voice=voice_name, language=language_code)
        response.append(gather)
        response.say(_get_prompt("reprompt"), voice=voice_name, language=language_code)
        return Response(content=str(response), media_type="application/xml")
...
async def voice_assist_type(request: Request) -> Response:
    ...
    choice = _resolve_assist_type(digits, speech_result)
    if not choice:
        log_event(call_sid, "INVALID_ASSIST_TYPE", {"digits": digits, "speech": speech_result})
        response = VoiceResponse()
        response.say(_get_prompt("invalid"), voice=voice_name, language=language_code)
        response.redirect("/voice")
        return Response(content=str(response), media_type="application/xml")
    ...
    if choice == "product":
        ...
        gather.say(_get_prompt("product_id_prompt"), voice=voice_name, language=language_code)
        response.append(gather)
        response.say(_get_prompt("reprompt"), voice=voice_name, language=language_code)
        return Response(content=str(response), media_type="application/xml")
    ...
    gather.say(_get_prompt("category_prompt"), voice=voice_name, language=language_code)
    response.append(gather)
    response.say(_get_prompt("reprompt"), voice=voice_name, language=language_code)
    return Response(content=str(response), media_type="application/xml")
...
async def voice_price_product(request: Request) -> Response:
    ...
    product_id = (digits or "" ).strip() or (speech_result or "").strip()
    if not product_id:
        response = VoiceResponse()
        response.say(_get_prompt("invalid"), voice=voice_name, language=language_code)
        response.redirect("/voice")
    ...
    # Ask for brief description, then dial
    response = VoiceResponse()
    gather = Gather(
        input="speech",
    ...
    response.append(gather)
    # If no speech captured, proceed to dial
    _append_dial_instruction(response, call_sid, getattr(get_lead_by_call_sid(call_sid), "selected_category", None), getattr(get_lead_by_call_sid(call_sid), "currency", None), language_code, from_number)
    return Response(content=str(response), media_type="application/xml")
...
async def voice_category_name(request: Request) -> Response:
    ...
    gather.say(_get_prompt("confirmation"), voice=voice_name, language=language_code)
    response.append(gather)
    _append_dial_instruction(response, call_sid, category, getattr(get_lead_by_call_sid(call_sid), "currency", None), language_code, from_number)
    return Response(content=str(response), media_type="application/xml")
...
async def voice_description(request: Request) -> Response:
    ...
    response = VoiceResponse()
    # proceed to dial using stored lead info
    lead = get_lead_by_call_sid(call_sid)
    category = getattr(lead, "selected_category", None)
    _append_dial_instruction(response, call_sid, category, getattr(lead, "currency", None), language_code, from_number)
    return Response(content=str(response), media_type="application/xml")
...
def _build_category_prompt(language_code: str, voice_name: str) -> VoiceResponse:
    ...
    gather.say(_get_prompt("menu"), voice=voice_name, language=language_code)
    response.append(gather)
    response.say(_get_prompt("reprompt"), voice=voice_name, language=language_code)
    return response
...
def _build_menu_prompt(language_code: str, voice_name: str) -> VoiceResponse:
    """Build the new initial menu prompt (intent selection)."""
    response = VoiceResponse()
    response.say(config_service.get_voice_greeting(language_code), voice=voice_name, language=language_code)
    gather = Gather(
        input="dtmf speech",
        ...
        language=SPEECH_RECOGNITION_LANGUAGE,
        timeout=6,
        hints="general inquiry, try near you, price request",
    )
...
def _resolve_intent(digits: str | None, speech: str | None) -> str | None:
    """Resolve initial intent from DTMF or speech.

    Returns one of: "general_inquiry", "store", "price_request"
    """
    if digits:
        d = digits.strip()
        if d == "1":
    ...
    if not transcript:
        return None
    # price keywords
    if any(k in transcript for k in ("price", "pricing", "cost", "rate")):
        return "price_request"
    # store keywords
    if any(k in transcript for k in ("store", "near", "nearby", "near you", "try near you")):
        return "store"
    # general inquiry
    if any(k in transcript for k in ("general", "inquiry", "enquiry", "help", "assist")):
        return "general_inquiry"
    return None


def _resolve_assist_type(digits: str | None, speech: str | None) -> str | None:
    ...
    if not transcript:
        return None
    if any(k in transcript for k in ("product", "id", "item", "sku")):
        return "product"
    if any(k in transcript for k in ("category", "categories", "type", "kind")):
        return "category"
    return None

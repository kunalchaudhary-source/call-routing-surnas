...
def pick_agent(category: str, currency: Optional[str]) -> Tuple[Optional[Agent], str]:
    ...
    try:
        ...
        fallback = settings.INDIA_AGENT_POOL if region == "IN" else settings.US_AGENT_POOL
...
def get_agent_candidates(category: str, currency: Optional[str], limit: int = 5) -> list[str]:
    ...
    try:
        ...
        # Finally add configured fallback pools
        pool = settings.INDIA_AGENT_POOL if region == "IN" else settings.US_AGENT_POOL
        if pool and pool not in candidates:
